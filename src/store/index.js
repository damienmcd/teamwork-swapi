      ...
      await axios
        ...
        .then((peopleData) => {
          ...
          if (peopleData.results.length) {
            ...
            commit('SET_PAGED_PEOPLE', { page: 1, people: peopleData.results, previous: peopleData.previous, next: peopleData.next });
            commit('SET_CURRENT_PAGE', { page: 1, previous: peopleData.previous, next: peopleData.next });
          }
        });
    },
      ...
      if (!state.pagedPeople.hasOwnProperty(payload.page)) {
        await axios
          ...
          .then((peopleData) => {
            if (peopleData.results.length) {
              commit('SET_CURRENT_PAGE_PEOPLE', peopleData.results);
              commit('SET_PAGED_PEOPLE', { page: payload.page, people: peopleData.results, previous: peopleData.previous, next: peopleData.next });
              commit('SET_CURRENT_PAGE', { page: payload.page, previous: peopleData.previous, next: peopleData.next });
            }
          });
      } else {
        console.log(state.pagedPeople[payload.page]);
        commit('SET_CURRENT_PAGE_PEOPLE', state.pagedPeople[payload.page].people);
        commit('SET_CURRENT_PAGE', { page: payload.page, previous: state.pagedPeople[payload.page].previous, next: state.pagedPeople[payload.page].next });
      }
    },
